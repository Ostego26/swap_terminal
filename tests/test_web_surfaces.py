...
def test_the_swap_form_offers_exactly_the_pairs_the_server_allows(client):
    ...
    # And nothing else. XRP, SOL and XMR are not in ALLOWED_PAIRS, so no option
    # for them may exist.
    for disabled_asset in ("XRP", "SOL", "XMR"):
        assert f'value="{disabled_asset}:' not in body
        assert f':{disabled_asset}"' not in body
